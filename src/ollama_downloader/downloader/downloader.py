...
class Downloader(ABC):
    ...
    def _make_manifest_url(
        ...
        match model_source:
            ...
            case ModelSource.HUGGINGFACE:
                ...
                return httpx.URL(
                    f"{self.HF_BASE_URL}{model_identifier.replace(':', '/manifests/')}"
                )
            case _:
    ...
    def _fetch_manifest(self, model_identifier: str, model_source: ModelSource) -> str:
        ...
        logger.info(f"Downloading manifest from [bold cyan]{url}[/bold cyan]")
    ...
    def _make_blob_url(
        ...
        match model_source:
            ...
            case ModelSource.HUGGINGFACE:
                return httpx.URL(f"{self.HF_BASE_URL}{model_name}/blobs/{digest}")
            case _:
                raise ValueError(f"Unsupported model source: {model_source}")
        ...
        match model_source:
            case ModelSource.OLLAMA:
                manifests_dir = os.path.join(
                    ...
                    urlparse(self.settings.ollama_library.registry_base_url).hostname,
                    "library",
